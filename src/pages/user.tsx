...
const IndexPage: FunctionComponent = () => {
  const [response, setResponse] = useState({ user: null, pinned_tweet: null })
  const [hasError, setHasError] = useState(false)
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    fetch(`${process.env.GATSBY_API_URL}/user`, {
      mode: 'no-cors',
      method: 'GET',
    })
  ...
  const { user, pinned_tweet } = response
          ...
          {!hasError && !isLoading && response ? (
            <>
              ...
              <Flex>
                <Card>
                  <Grid sx={{ gap: 2 }}>
                    <Grid sx={{ alignItems: 'center', gap: 2, gridTemplateColumns: 'auto 1fr' }}>
                      ...
                      <Heading as="h3">Pinned Tweet</Heading>
                    </Grid>
                    <Text sx={{ fontSize: 0 }}>{pinned_tweet.text}</Text>

                    <Grid sx={{ gap: 2, gridTemplateColumns: 'auto auto 1fr' }}>
                      <Text sx={{ fontSize: 0 }}>
                        Retweets:{' '}
                        <Text as="span" sx={{ fontWeight: 'bold' }}>
                          {pinned_tweet.public_metrics.retweet_count}
                        </Text>
                      </Text>
                      <Text sx={{ fontSize: 0 }}>
                        Likes:{' '}
                        <Text as="span" sx={{ fontWeight: 'bold' }}>
                          {pinned_tweet.public_metrics.like_count}
                        </Text>
                      </Text>
                      <Text sx={{ fontSize: 0 }}>
                        Comments:{' '}
                        <Text as="span" sx={{ fontWeight: 'bold' }}>
                          {pinned_tweet.public_metrics.reply_count}
                        </Text>
                      </Text>
                    </Grid>
                  </Grid>
                ...
                {/* <pre>
                  <code>{JSON.stringify(pinned_tweet, null, 2)}</code>
                </pre> */}
              </Flex>
            </>
          ) : null}
